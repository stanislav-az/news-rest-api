...
userAssignRoute =
  PathRoute "api" $
  PathRoute "users" $
  DynamicRoute "pk" $ PathRoute "assign" $ MethodRoute "POST"

userRejectRoute :: Route
userRejectRoute =
  PathRoute "api" $
  PathRoute "users" $
  DynamicRoute "pk" $ PathRoute "reject" $ MethodRoute "POST"
...
spec = do
  describe "checkout" $ do
    ...
    it "should not route to non-existing GET url" $ do
      ...
      (fst res) `shouldBe` False
    it
      "should route to user assign page for user 55 with POST and get dynamic path" $ do
      let res =
            checkout [] userAssignRoute ["api", "users", "55", "assign"] "POST"
      res `shouldBe` (True, [("pk", "55")])
    it
      "should route to user reject page for user 55 with POST and get dynamic path" $ do
      let res =
            checkout [] userRejectRoute ["api", "users", "55", "reject"] "POST"
      res `shouldBe` (True, [("pk", "55")])
    it
      "should route to user assign page with / for user 55 with POST and get dynamic path" $ do
      let res =
            checkout
              []
              userAssignRoute
      ...
      res `shouldBe` (True, [("pk", "55")])
    it
      "should route to user reject page with / for user 55 with POST and get dynamic path" $ do
      let res =
            checkout
              []
              userRejectRoute
    ...
    it "should not route if has wrong sub-path" $ do
      let res =
            checkout [] userRejectRoute ["api", "user", "55", "reject"] "POST"
      (fst res) `shouldBe` False
  ...
  describe "route" $ do
    ...
    it "should call create author handler on POST /api/author" $ do
      let authorReq =
            defaultRequest
              {requestMethod = "POST", pathInfo = ["api", "author"]}
      res <- route routes authorReq f
      responseStatus res `shouldBe` HTTP.status200
    it "should not call create author handler on methods other than POST" $ do
      let authorReq =
            defaultRequest {requestMethod = "GET", pathInfo = ["api", "author"]}
      res <- route routes authorReq f
      responseStatus res `shouldBe` HTTP.status404
    it "should not call create author handler on incorrect path" $ do
      let authorReq =
            defaultRequest
              {requestMethod = "POST", pathInfo = ["api", "authors"]}
      res <- route routes authorReq f
      responseStatus res `shouldBe` HTTP.status404
    it "should not call any handlers and respond with 404 on empty routes" $ do
      res <- route [] defaultRequest f
      responseStatus res `shouldBe` HTTP.status404
    it "should call first handler on duplicate routes" $ do
      let routesDuplicate =
            [ (createAuthorRoute, pure responseOk)
            , (createAuthorRoute, error "Second route should not be called")
            ]
          authorReq =
            defaultRequest
              {requestMethod = "POST", pathInfo = ["api", "author"]}
