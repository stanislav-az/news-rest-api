...
import qualified Data.Map.Strict as M (Map(..), elems, keys, lookup)
...
newtype MockMonad a = MockMonad
  { runMockMonad :: StateT MockIO (ReaderT HM.HandlerEnv (Except HM.HandlerError)) a
             ...
             , Applicative
             , Monad
             , MonadReader HM.HandlerEnv
             , MonadError HM.HandlerError
             , MonadState MockIO
             )
...
data MockDB =
  DB (M.Map Integer User)
     (M.Map Integer (Author, User))
     (M.Map Integer Tag)
     (M.Map Integer CategoryNested)
     (M.Map Integer Commentary)
  deriving (Show)
...
instance MD.PersistentCategory MockMonad where
  ...
  insertCategory CategoryRaw {..} = do
    db <- gets mockDB
    let eParent = maybe (Left "no parent") (`selectById` db) categoryRawParentId
    pure $
      Right $ either (const root) (CategoryNested 2 categoryRawName) eParent
    where
      root = Parent 1 categoryRawName
