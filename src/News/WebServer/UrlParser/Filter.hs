...
module News.WebServer.UrlParser.Filter where

import Control.Applicative ((<|>))
import qualified Data.ByteString as B (ByteString(..))
import qualified Data.Text as T (Text(..))
import qualified Data.Time as Time (Day(..))
import qualified Network.Wai as W (Request(..))
import News.WebServer.UrlParser.Query (getQueryBS, getQueryParam)
...
data DateCreated
  = CreatedAt Time.Day
  | CreatedAtLt Time.Day
  | CreatedAtGt Time.Day
  deriving (Show)
...
getFilter req =
  Filter
    { filterDateCreated =
        (CreatedAt <$> getQueryParam req "created_at") <|>
    ...
    , filterTagIds =
        (TagId <$> getQueryParam req "tag_id") <|>
