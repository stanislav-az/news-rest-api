...
module News.WebServer.Database
  ( Limit(..)
  , Offset(..)
  , Persistent(..)
  , Fit(..)
...
import qualified Database.PostgreSQL.Simple.ToRow as PSQL (ToRow(..), toRow)
...
class Persistent entity where
  ...
  default select :: (PSQL.FromRow entity) =>
    PSQL.Connection -> (Limit, Offset) -> IO [entity]
  select conn (Limit limit, Offset offset) =
    PSQL.query conn dbQuery [limit, offset]
    where
      dbQuery =
        "SELECT * FROM " <> tableName (P.Proxy :: P.Proxy entity) <>
        " LIMIT ? OFFSET ? ;"
  selectById :: PSQL.Connection -> Integer -> IO (Maybe entity)
  default selectById :: (PSQL.FromRow entity) =>
    PSQL.Connection -> Integer -> IO (Maybe entity)
...
class (Persistent stored) =>
      Fit object stored
  where
  insert :: PSQL.Connection -> object -> IO (Maybe stored)
  default insert :: (PSQL.FromRow stored, PSQL.ToRow object) =>
    PSQL.Connection -> object -> IO (Maybe stored)
  insert conn item = MB.listToMaybe <$> PSQL.query conn dbQuery item
    where
      placeholders = toPlaceHoldersList item
      dbQuery =
        "INSERT INTO " <> tableName (P.Proxy :: P.Proxy stored) <> " VALUES " <>
        placeholders <>
        " RETURNING * ;"
